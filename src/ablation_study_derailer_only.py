...
if __name__ == '__main__':
    ...
    df = pd.read_csv('../data/final_test_data/added_experiments/final_result_gpt4.csv')
    ...
    if len(result_df_dict['CaseID']) > 1:
        result_df = pd.DataFrame.from_dict(result_df_dict)
        result_df.to_csv('../result/ablation_study/aba_df_derail_only.csv', mode='a', header=not header_written, index=False)
